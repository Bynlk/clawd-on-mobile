...
- IPC 与持久化实现：注册 `wgRelay:deploy`、`connect`、`disconnect`、`rotate-phone`、`delete-local`、`pairing-qr`、`status`、`list-statuses` 八个新合同，并仅保留旧 tunnel handler 兼容。profile 先经 canonical public sanitize；SSH 密码只存在于单次调用参数并在 `finally` 清除。未知 SSH host key 只可经注入的 Electron dialog 明确确认 TOFU，已保存指纹不匹配由部署层直接拒绝。严格 readback 的 pc/phone config、Relay Token 与 management Token 先写 safeStorage secret store 并 readback 验证，再由 settingsController 唯一写入 public profile；secret/public 任一步异常均恢复旧本地快照。成功必须完成 Task 5 connection manager connect 和 version 1 QR 编码后才返回，renderer 只收到 canonical public profile、脱敏状态和 PNG data URL。
- rotate 与配对实现：手机轮换先确保 PC tunnel connected，只向 Task 5 sidecar 暴露的 loopback forward endpoint 发送固定小型 JSON POST；请求禁用连接池、拒绝 redirect、限制 deadline/响应大小/content-type，使用 management Bearer 并严格接受 `{version,phoneConfig,relayToken}`。新 secret 写入验证后重启并验证 PC connection，成功才替换 QR cache；失败恢复旧本地 secret/connection/QR。QR 使用仓库既有 `qrcode` 依赖编码 `clawd://relay-pair?v=1&data=<base64url>`，deep link 限制 8 KiB，renderer 永不获得 raw payload。
- 生命周期与最小 preload：preload 只暴露八个固定 invoke 方法和 status/progress 两个可取消 listener，参数、结果和事件均跨 clone 边界，不暴露 `ipcRenderer` 或 raw channel。main 仅在 app ready 后按 `userDataPath/resourcesPath/isPackaged/platform/arch` 创建 safeStorage store、sidecar factory、Task 5 connection manager 与 IPC，不自动连接；safeStorage unavailable 和 Linux `basic_text` 均保留 IPC 但稳定 fail closed。quit barrier 在首次 `before-quit` 调用 `preventDefault()`，等待幂等 dispose 完成后再触发一次 `app.quit()`；`will-quit` 复用同一 disposal，避免 listener、connection 和 sidecar 重复释放。
- 审查回归 RED：双审查后运行 `node --test test/wg-relay-ipc.test.js test/main-wg-relay-integration.test.js`，退出码 1，共 22 项，15 通过、7 失败；失败覆盖 secret write 成功但验证 read 抛错时旧值未恢复、dispose 期间晚到 QR 回填 cache、public 删除失败却隐藏仍存在的 profile、safeStorage 可用性重复读取、以及 Electron quit 未等待异步 dispose。`node --test --test-name-pattern='controller mutates' test/wg-relay-ipc.test.js` 退出码 1，0/1 通过，证明 settingsController 已更新内存但持久化返回 error 时 public profile 未回滚；`node --test --test-name-pattern='delete-local disconnects' test/wg-relay-ipc.test.js` 退出码 1，0/1 通过，证明成功删除后单条 status 仍暴露旧 runtime snapshot。
- 审查修复 GREEN：secret/public 写入在调用可能产生副作用前即标记 rollback ownership；dispose 等待 per-profile tail 后二次清空 QR；异步 QR 编码完成后再次检查 disposed；部分删除只在 public profile 实际移除后隐藏状态，成功删除后的单条 status 返回稳定 `profile_not_found`。相同 22 项审查组合修复后 22/22 通过；两个单项回归命令分别 1/1 通过。
- 双审查结论：两位独立 reviewer 均完成窄范围审查。有效的本地原子性、late event/cache、部分删除一致性和 Electron 异步退出问题已按 RED→GREEN 修复。关于“绕过 Task 5 manager”的意见经核对不成立：main integration 创建并持有的正是 `createWgRelayConnection()`。两位 reviewer 均指出远端部署/手机轮换已经提交后，本地失败无法凭本地旧快照恢复远端；本 Task 按明确合同继续执行旧 secrets/连接/QR 的本地回滚，但该协议不可逆窗口保留为后续风险，若要闭合需管理 API 提供 CAS/补偿事务，而不能由 Electron 单边保证。
- Task 6 指定 GREEN：`node --test test/wg-relay-ipc.test.js test/wg-relay-preload.test.js test/main-wg-relay-integration.test.js test/settings-ipc.test.js` 退出码 0，38/38 通过，0 失败、0 跳过。额外 `node --test test/wg-relay-pairing-qr.test.js` 退出码 0，3/3 通过。
- Task 1–5 WG/Relay/settings 邻接 GREEN：`node --test test/wg-relay-profile.test.js test/wg-relay-secret-store.test.js test/settings-actions-wg-relay.test.js test/wg-relay-bundle.test.js test/wg-ssh2-exec.test.js test/wg-relay-deploy.test.js test/relay-auth-management.test.js test/relay-managed-session-forwarding.test.js test/relay-server-bind.test.js test/wg-relay-sidecar.test.js test/wg-relay-connection.test.js test/relay-bridge-integration.test.js test/wg-relay-runtime.test.js test/wg-relay-ipc.test.js test/settings-ipc.test.js` 退出码 0，413/413 通过，0 失败、0 跳过。
