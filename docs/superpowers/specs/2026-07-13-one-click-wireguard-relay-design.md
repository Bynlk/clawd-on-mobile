...
- durable recovery journal：secret store 向后兼容读取 v1，并在下一次写时原子升级为 v2 `{version:2,profiles,recovery}`；两个 map 都是 profileId → 独立 safeStorage 密文 blob，磁盘不含 private key/token/raw candidate。journal API 支持 write/read/remove/list，写入使用 `0600` temp、文件 fsync、rename，并在 POSIX 上继续 fsync `userDataPath` 父目录以持久化目录项。主 secrets write+readback 与必要 public profile write 都成功后才删除 journal；candidate journal 更新失败时 durable prepared 仍保留，当前进程另用 `recoveryBundles` 暂存 candidate。invalid topology/schema 也把受限 raw candidate 标记为 `remote_committed_invalid_response`，renderer 永远只得到固定 `remote_commit_recovery_required`，拿不到 raw。
...
- delete/quit：delete-local 串行执行 disconnect → secret/public/QR/durable journal/runtime 清理；即使 secret 删除本身部分失败，也明确删除 journal 和内存 recovery，排队中的 connect/pairing/rotate 出队后重新检查 deleted tombstone。runtime 的单 profile `removeStatus()` 清 status 与 pcConfig；删除后的 late status event 会被 tombstone 同步再次清除。正常 quit 会尽力前滚当前进程 candidate，但 flush 只是 durable journal 的补充；prepared/invalid 已安全落盘时不得因为无法自动前滚而永久阻止退出，journal 保留供下次启动继续 fail closed。
...
- 最终 GREEN：Task 6 + deploy/secret/main/QR/settings 精确命令 `node --test test/wg-relay-ipc.test.js test/wg-relay-secret-store.test.js test/wg-relay-preload.test.js test/main-wg-relay-integration.test.js test/wg-relay-pairing-qr.test.js test/settings-ipc.test.js test/wg-relay-deploy.test.js` 为 190/190、0 失败、0 跳过；真实 Bridge 筛选 `node --test --test-name-pattern='real RelayBridge' test/main-wg-relay-integration.test.js` 为 1/1。Task 1–5 WG/Relay/settings 邻接 15 文件命令为 451/451、0 失败、0 跳过。对本轮 7 个 changed JS 执行 `node --check` 全部退出码 0，`git diff --check` 退出码 0。首轮只读 reviewer 报告无 Critical、4 个 Important（完整 deploy schema、durable blocked quit、错误码稳定、父目录 fsync）；最终只读 reviewer 再报告 2 项（超限 committed candidate 未清旧状态、单 profile recovery read failure 误伤全局），六项均按新增 RED→GREEN 修复，先前 reviewer 问题继续保持 GREEN，无遗留 Critical/Important。
