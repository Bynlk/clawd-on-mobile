...
- Task 4 当前状态：两项复审缺陷已修复并通过当前自动验证，不再以百分比宣称完成。改动仍仅限 `sidecars/wg-relay-tunnel` 授权文件与本进度记录；依赖版本未变化，`NOTICE.md` 的既有实际许可证清单仍准确，未修改 Task 5+ 文件，未读取或使用任何真实 VPS 凭据，也未连接 VPS。整个一键 Relay 设计仍需后续 Task 5–12 才能达到第 14 节的全功能完成定义。
