...
- 事务恢复与并发边界 RED：token/env 外部字段更新、stale CAS、`0644` env、竞争目录锁、live rollback 失败、file rollback 失败、live verification mismatch、hung key/command、partial HTTP body、hung handler、forced close 以及 upgrade 前认证/限流筛选均先出现定向失败；修复后 Node management 与 token store 共用 `/run/lock/clawd-relay.lock` 的有界 mkdir 锁，轮换队列和跨进程锁共同覆盖完整 snapshot→commit/compensate，env 在锁内 reload+CAS，补偿失败稳定返回 `rollback_failed` 并保持 unhealthy，只有显式认证 status 恢复且验证旧 live peer 后才能清除。生产命令、请求读取/处理和 server shutdown 均有 deadline；非法 path/role/Bearer/来源限流在 `handleUpgrade` 前返回 403/401/429，CLI 默认拒绝仅靠临时 `RELAY_TOKEN` 启动。
