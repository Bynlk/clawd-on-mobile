...
- 最终聚焦验证：在 `android/` 执行 `JAVA_HOME=/opt/homebrew/opt/openjdk@17 ANDROID_HOME=$HOME/.local/android ./gradlew testDebugUnitTest --tests '*RelayPairing*' --tests '*PrefsStoreTest*`，退出码 0，共 70 项，70 通过、0 失败、0 跳过；组成是 parser 11、跨语言/路由 integration 10、导航 7、PrefsStore 42。
- Android 邻接/编译/lint 验证：相同环境执行最终组合命令 `./gradlew testDebugUnitTest lintDebug assembleDebug`，退出码 0；完整单测共 621/621，`lintDebug` 生成 debug lint HTML且 `assembleDebug` 成功产出 debug APK。PC 邻接 `node --test test/wg-relay-pairing-qr.test.js` 为 7/7、0 失败；提交前另执行 `git diff --check` 与授权范围检查。
