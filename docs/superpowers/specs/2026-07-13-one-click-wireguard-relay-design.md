...
- 严格 schema 审查 RED/GREEN：独立审查指出 `encoding/json` 默认接受重复字段和大小写不精确字段；新增回归先观察到 ambiguous field set 被接受，再改为先读取单个 `json.RawMessage`、逐 token 要求七个字段精确各出现一次，并继续在实际 struct decoder 上调用 `DisallowUnknownFields()`。多行 JSON 首先返回 `invalid_json`、无末尾换行 EOF 首先 2 秒不退出；有界 streaming decoder 修复后两项通过，且运行期延迟第二 JSON 仍返回脱敏 `trailing_data` 并清理。
...
- 第二轮独立复审未发现 Task 4 内部新的安全或并发缺陷，但指出 Task 5 旧计划的“发送配置后关闭 stdin”与本次明确的“stdin EOF 有界退出并清理”要求冲突。本实现按 Task 4 最新要求在 EOF 后关闭 listener/device；后续 Task 5 manager 必须在 sidecar 存活期间保持 stdin 打开，不能沿用旧的 `stdin.end()` 假设。本次不越界修改 Task 5 文件。
...
- RelayBridge/runtime 实现：RelayBridge 新增显式 `configure({url,token})`、幂等 `start()`/`waitUntilConnected(timeoutMs)`/`stop()`，Relay 与本地 mobile socket 均 open 后才 connected；认证/连接/超时使用稳定码，旧 socket 晚事件受 generation 和 socket identity 双重门禁，Token 不进入日志或 failure。legacy `init(prefs)` 保留且清理 prefs listeners。runtime 保留旧 deploy/tunnel 状态兼容，同时公开 `starting_tunnel`、`verifying_relay`、`connecting_relay`、`disconnecting` 等一键状态，只接受稳定 errorCode、公开字段白名单和单调 attempt。
...
- 遗留互操作风险：本 Task 按本轮明确要求在写入一个 JSON 后关闭 stdin；Task 4 当前 sidecar 则把 stdin EOF 定义为立即清理并退出。两项契约在真实二进制上冲突，Task 5 的 fake-child 单元测试不能证明隧道会持续存活；后续集成前必须统一协议（保持 stdin 打开，或修改 sidecar 生命周期协议）并增加真实进程 smoke。Windows 仍只有可测试 kill 分支，没有实机 runtime smoke；该项继续等待 Task 11 CI。
