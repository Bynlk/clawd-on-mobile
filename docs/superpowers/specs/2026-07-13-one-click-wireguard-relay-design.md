...
- IPC 与边界：注册 `wgRelay:deploy`、`connect`、`disconnect`、`rotate-phone`、`delete-local`、`pairing-qr`、`status`、`list-statuses` 八个新合同，并仅保留旧 tunnel handler 兼容。profile 先经 canonical public sanitize；SSH password 只存在于单次调用参数并在 `finally` 清除。未知 SSH host key 只可经注入的 Electron dialog 明确确认 TOFU，已保存指纹不匹配直接拒绝。settingsController 是唯一 public profile writer；renderer 只收到 public profile、PNG data URL 和仅含 `profileId/status/generation/errorCode` 的值级 allowlist 状态，固定 progress 阶段同样不复制 message/hint/token-like 字段。
- 不可逆 commitPoint：deploy 以 `deployFn` 返回 `ok + readback` 为远端提交点；rotate 以管理 POST 收到 HTTP 2xx 为远端提交点，即使之后响应体超限、截断或 schema 无效也不得按提交前失败处理。每次远端请求前执行 safeStorage encrypt/decrypt roundtrip 与真实 temp/chmod/fsync/rename 可写 preflight。提交点前失败保持旧状态；提交点后只前滚，先将新 bundle 写入 encrypted secret store 并 readback 验证，再写 public profile、释放旧连接、用新配置连接并生成 QR，绝不恢复旧 secrets、旧 token 或旧 QR。
- recovery 与部分成功：post-commit write/readback 失败把完整新 bundle 保留在受控进程内 `recoveryBundles`，`deploy` retry、`connect`、`pairing-qr` 和 rotate 入队执行时均先冲刷 recovery，并重新读取冲刷后的当前 profile。稳定结果使用 `partial_success + retryable` 与 `local_storage_retry_required`、`public_profile_retry_required`、`connection_retry_required`、`pairing_qr_retry_required`；远端已提交但无法信任响应时建立 `remote_committed_invalid_response` blocked tombstone，清旧 QR 并释放旧连接。若旧连接首次释放失败，单 profile release-required tombstone 强制后续 connect 先再次 disconnect，不能由 Task 5 manager 复用旧 token 的 connected record。
- rotate 与严格配对模型：rotate 仅通过 sidecar loopback forward endpoint 调用 WireGuard 内网管理 API，Bearer、deadline、请求/响应大小、content-type、no redirect 和关闭连接均有界。deploy readback 与 rotate response 共用严格 phone parser：精确一个 `[Interface]`/`[Peer]`，拒绝重复/未知字段，校验 canonical 非零 WG key、profile `/24` 推导的 `.3/32`、精确私网 AllowedIPs、合法 domain/IPv4/括号 IPv6 endpoint 与 profile endpoint、Keepalive 25，以及固定 `.1:7891` Relay URL。QR 使用既有 `qrcode` 生成 `clawd://relay-pair?v=1&data=<base64url>`，总长限制 8 KiB；真实 PNG 测试用 `pngjs` 解码像素并逐 module 对照 `qrcode.create()` 的标准矩阵，renderer 不获得 raw payload。
...
- delete/quit：delete-local 串行执行 disconnect → secret/public/QR/recovery/runtime 清理；partial delete 保留 tombstone，排队中的 connect/pairing/rotate 出队后重新检查，不能恢复旧 secret。runtime 的单 profile `removeStatus()` 清 status 与 pcConfig；删除后的 late status event 会被 tombstone 同步再次清除。正常 quit 先等待 operation tails，再最终冲刷所有 recovery；若持久化仍失败，quit barrier 保持进程存活并允许下一次退出重试，成功后才移除 IPC/listener 并继续 `app.quit()`。
- 极端恢复边界：preflight 只能显著缩小、不能消除远端提交后磁盘立即失效的窗口。新 bundle 在本进程仍存活时由 recovery map 保护，正常退出会强制最终落盘；若磁盘持续不可写且进程随后被强杀、崩溃、断电或 OS 终止，尚未 durable 的内存 bundle 会丢失，Electron 单边无法从已轮换的远端恢复。这是当前明确且唯一接受的极端边界；不会以恢复旧本地凭据伪装成功。
...
- 最终 GREEN：Task 6 指定命令 `node --test test/wg-relay-ipc.test.js test/wg-relay-preload.test.js test/main-wg-relay-integration.test.js test/settings-ipc.test.js` 为 54/54；QR 单文件为 6/6；真实 Bridge 筛选 `node --test --test-name-pattern='real RelayBridge' test/main-wg-relay-integration.test.js` 为 1/1；加入 secret/runtime/Bridge/connection 的 9 文件扩大组合为 153/153，均为 0 失败、0 跳过。Task 1–5 WG/Relay/settings 邻接 15 文件命令为 431/431、0 失败、0 跳过。对本轮 13 个 changed JS 执行 `node --check` 全部退出码 0，`git diff --check` 退出码 0。两位只读 reviewer 最终报告的 6 个 Important（2xx commit 误判、legacy token snapshot、队列旧 profile、partial delete 队列绕过、正常 quit 丢 recovery、progress/late runtime）均已按新增 RED→GREEN 修复，无遗留 Critical/Important。
