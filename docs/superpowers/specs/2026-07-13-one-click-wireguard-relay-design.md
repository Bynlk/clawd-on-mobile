...
- 最终 GREEN：Task 6 + deploy/secret/main/QR/settings 精确命令 `node --test test/wg-relay-ipc.test.js test/wg-relay-secret-store.test.js test/wg-relay-preload.test.js test/main-wg-relay-integration.test.js test/wg-relay-pairing-qr.test.js test/settings-ipc.test.js test/wg-relay-deploy.test.js` 在 delete_pending 复审后为 194/194、0 失败、0 跳过；真实 Bridge 筛选 `node --test --test-name-pattern='real RelayBridge' test/main-wg-relay-integration.test.js` 为 1/1。Task 1–5 WG/Relay/settings 邻接 15 文件命令随同一 IPC 文件新增四项更新为 455/455、0 失败、0 跳过（原 451 项全部继续通过）。对本轮 changed JS 执行 `node --check` 全部退出码 0，`git diff --check` 退出码 0。首轮只读 reviewer 报告无 Critical、4 个 Important（完整 deploy schema、durable blocked quit、错误码稳定、父目录 fsync）；最终只读 reviewer 再报告 2 项（超限 committed candidate 未清旧状态、单 profile recovery read failure 误伤全局），六项均按新增 RED→GREEN 修复，先前 reviewer 问题继续保持 GREEN，无遗留 Critical/Important。
