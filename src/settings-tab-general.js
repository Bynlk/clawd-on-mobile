...
(function initSettingsTabGeneral(root) {
  ...
  const FLASH_NUMBER_KEYS = new Set(["flashIntervalMs", "flashDurationMs"]);
  ...
  function render(parent) {
    ...
    parent.appendChild(
      helpers.buildSection(t("sectionAppearance"), [
        buildLanguageRow(),
        buildSizeSliderRow(),
        buildTextScaleRow(),
      ]),
    ...
    parent.appendChild(
      helpers.buildSection(t("sectionSession"), [
    ...
    parent.appendChild(
      helpers.buildSection(t("sectionAlerts"), [
        buildSoundGroup(),
        buildFlashGroup(),
      ]),
    );
    ...
    parent.appendChild(
      helpers.buildSection(t("sectionBubbles"), [
        helpers.buildSwitchRow({
          key: "hideBubbles",
          labelKey: "rowHideBubbles",
          descKey: "rowHideBubblesDesc",
          onToggle: ({ nextRaw }) =>
            window.settingsAPI.command("setAllBubblesHidden", {
              hidden: nextRaw,
            }),
        }),
    ...
    parent.appendChild(
      helpers.buildSection(t("sectionBehavior"), [
        helpers.buildSwitchRow({
          key: "freeRoam",
    ...
    parent.appendChild(
      helpers.buildSection(t("sectionSystemStartup"), [
        helpers.buildSwitchRow({
    ...
    parent.appendChild(
      helpers.buildSection(t("sectionPermissions"), [
        helpers.buildSwitchRow({
  ...
  function buildTutorialReplayHint() {
    ...
    button.addEventListener("click", () => {
      if (
        !window.settingsAPI ||
        typeof window.settingsAPI.showTutorial !== "function"
      )
        return;
      ...
      window.settingsAPI
        ...
        .then((result) => {
          if (!result || result.status !== "ok") {
            throw new Error(
              (result && result.message) || t("settingsTutorialReplayFailed"),
            );
          }
        })
        .catch((err) => {
          const message =
            t("settingsTutorialReplayFailed") +
  ...
  function confirmAutoApproveAll(nextRaw) {
    ...
    if (!nextRaw)
      return window.settingsAPI.command("setAutoApproveAll", {
        enabled: false,
      });
  ...
  function showAutoApproveAllConfirmModal() {
    return helpers.showSettingsConfirmModal({
      ...
      actions: [
        {
          id: "enable",
          label: t("autoApproveAllConfirmEnable"),
          tone: "danger",
        },
        {
          id: "cancel",
          label: t("autoApproveAllConfirmCancel"),
          tone: "accent",
          defaultFocus: true,
        },
      ],
  ...
  function buildDashboardRow() {
    ...
    btn.addEventListener("click", () => {
      if (
        window.settingsAPI &&
        typeof window.settingsAPI.openDashboard === "function"
      ) {
  ...
  function buildLanguageRow() {
    ...
    function getOption(lang) {
      return (
        options.find((option) => option.dataset.lang === lang) ||
        options[0] ||
        null
      );
    }
    function syncDisplay(lang) {
      const selectedLang = LANGUAGE_OPTIONS.includes(lang)
        ? lang
        : LANGUAGE_OPTIONS[0];
    ...
    function chooseLanguage(next) {
      ...
      window.settingsAPI
        ...
        .catch((err) => {
          ops.showToast(t("toastSaveFailed") + (err && err.message), {
            error: true,
          });
          revertIfStillPending();
        });
  ...
  function buildSessionHudGroup() {
    ...
    const sessionHudControlsEnabled = !!(
      state.snapshot && state.snapshot.sessionHudEnabled
    );
  ...
  function buildSessionHudSummary() {
    ...
    function syncFromSnapshot() {
      ...
      for (const item of items) {
        ...
        chip.className =
          "collapsible-summary-chip" + (item.accent ? " accent" : "");
  ...
  function buildSessionCleanupGroup() {
    const optionList = buildOptionList("session-cleanup-option-list", [
      ...
      helpers.buildNumberInputRow({
        ...
        fromDisplay: (sec) =>
          Math.max(30_000, Math.min(86_400_000, Math.round(sec * 1000))),
        min: 30,
        max: 86_400,
      }).row,
      helpers.buildNumberInputRow({
        ...
        toDisplay: (ms) => Math.round(ms / 1000),
        fromDisplay: (sec) =>
          Math.max(5_000, Math.min(300_000, Math.round(sec * 1000))),
        min: 5,
        max: 300,
      }).row,
    ]);
  ...
  function buildSoundGroup() {
    ...
    return helpers.buildCollapsibleGroup({
      ...
      children: [
        buildOptionList("sound-option-list", [
  ...
  function buildFlashGroup() {
    return helpers.buildCollapsibleGroup({
      ...
      children: [
        buildOptionList("flash-option-list", [
          helpers.buildSwitchRow({
          ...
          helpers.buildNumberInputRow({
            ...
            fromDisplay: (v) => {
              ...
              return Number.isFinite(n)
                ? Math.max(0, Math.min(60000, Math.round(n)))
                : 0;
            },
            min: 0,
            max: 60000,
  ...
  function buildSoundEnabledRow(summaryControl) {
    ...
    const visualOn = override
      ? override.visualOn
      : readers.readGeneralSwitchVisual("soundMuted", true);
    helpers.setSwitchVisual(sw, visualOn, {
    ...
    const run = (ev) => {
      if (
        sw.classList.contains("disabled") ||
        sw.getAttribute("aria-disabled") === "true"
      )
        return;
      if (!summaryControl || typeof summaryControl.toggleSound !== "function")
        return;
  ...
  function buildSoundSummary() {
    ...
    function getSnapshotVolumePct() {
      const v =
        state.snapshot && typeof state.snapshot.soundVolume === "number"
          ? state.snapshot.soundVolume
          : 1;
    ...
    function setSoundChildSwitchVisual(visualOn, pendingVisual) {
      ...
      if (!meta) return;
      helpers.setSwitchVisual(meta.element, visualOn, {
    ...
    function applySoundSummaryVisual(
      enabled,
      pendingVisual = false,
      volumePct = getSnapshotVolumePct(),
    ) {
      const stateLabel = enabled
        ? t("bubblePolicySummaryOn")
        : t("bubblePolicySummaryOff");
    ...
    function syncFromSnapshot() {
      applySoundSummaryVisual(
        getDisplaySoundVisual(),
        getDisplaySoundPending(),
      );
    }
  ...
  function readBubblePolicySnapshot() {
    const aggregateHidden = !!(
      state.snapshot && state.snapshot.hideBubbles === true
    );
    return {
      permissionOn:
        !aggregateHidden &&
        !!(state.snapshot && state.snapshot.permissionBubblesEnabled !== false),
      notificationSeconds: aggregateHidden
        ? 0
        : Number(
            state.snapshot && state.snapshot.notificationBubbleAutoCloseSeconds,
          ) || 0,
      updateSeconds: aggregateHidden
        ? 0
        : Number(
            state.snapshot && state.snapshot.updateBubbleAutoCloseSeconds,
          ) || 0,
    };
  }
  ...
  function buildBubblePolicySummary() {
    ...
    function syncFromSnapshot() {
      ...
      const items = [
        ...
        {
          text: t("bubblePolicySummaryNotification").replace(
            "{seconds}",
            String(snapshot.notificationSeconds),
          ),
          accent: snapshot.notificationSeconds > 0,
        },
        {
          text: t("bubblePolicySummaryUpdate").replace(
            "{seconds}",
            String(snapshot.updateSeconds),
          ),
  ...
  function buildBubbleCategoryControl({
    category,
    labelKey,
    descKey,
    warningKey = null,
    ...
    function currentEnabled() {
      ...
      if (enabledKey)
        return !!(state.snapshot && state.snapshot[enabledKey] !== false);
      if (!secondsKey)
        return !!(
          state.snapshot && state.snapshot.permissionBubblesEnabled !== false
        );
    ...
    function syncFromSnapshot() {
      ...
      if (document.activeElement === secondsInput || secondsDraftValue != null)
        return;
    ...
    function submitSecondsCommit(next) {
      ...
      return commitSecondsValue(secondsInput, secondsKey, next, category).then(
        (committed) => {
          if (seq === secondsCommitSeq && secondsInFlightValue === next)
            secondsInFlightValue = null;
    ...
    function flushSecondsCommit() {
      ...
      if (next == null) {
        ...
        secondsInput.value = String(
          Number(state.snapshot && state.snapshot[secondsKey]) || 0,
        );
        ops.showToast(t("toastSaveFailed") + t("bubbleSecondsInvalid"), {
          error: true,
        });
        return;
      }
    ...
    function runToggleCommit(nextEnabled) {
      setVisual(nextEnabled, true);
      window.settingsAPI
        .command("setBubbleCategoryEnabled", { category, enabled: nextEnabled })
        .then((result) => {
          if (!result || result.status !== "ok") {
            setVisual(currentEnabled(), false);
            const msg = (result && result.message) || "unknown error";
  ...
  function confirmDisableUpdateBubbles() {
    return helpers.showSettingsConfirmModal({
      ...
      actions: [
        {
          id: "confirm",
          label: t("updateBubbleDisableConfirmAction"),
          tone: "danger",
        },
        {
          id: "cancel",
          label: t("updateBubbleDisableConfirmCancel"),
          tone: "accent",
          defaultFocus: true,
        },
      ],
  ...
  function buildVolumeSliderRow() {
    ...
    function applySliderValue(pct) {
      ...
      if (
        summary &&
        document.body.contains(summary.element) &&
        typeof summary.syncVolumePreview === "function"
      ) {
  ...
  function buildTextScaleRow() {
    ...
    function syncFromContext() {
      if (
        !window.settingsAPI ||
        typeof window.settingsAPI.getTextScaleContext !== "function"
      )
        return;
      Promise.resolve(window.settingsAPI.getTextScaleContext())
        .then((context) => {
          const pct =
            context && Number.isFinite(Number(context.percent))
              ? Number(context.percent)
              : 100;
          paint(
            Math.min(
              TEXT_SCALE_UI_MAX,
              Math.max(TEXT_SCALE_UI_MIN, Math.round(pct)),
            ),
          );
        })
        .catch(() => {});
    ...
    function paint(pct) {
      slider.value = String(pct);
      const fill =
        ((pct - TEXT_SCALE_UI_MIN) / (TEXT_SCALE_UI_MAX - TEXT_SCALE_UI_MIN)) *
        100;
    ...
    function snapTextScalePct(raw) {
      ...
      const stepped =
        TEXT_SCALE_UI_MIN +
        Math.round((base - TEXT_SCALE_UI_MIN) / TEXT_SCALE_UI_STEP) *
          TEXT_SCALE_UI_STEP;
    ...
    function commit(pct) {
      window.settingsAPI
        .command("setTextScaleForDisplay", { value: pct / 100 })
        .then((result) => {
          if (!result || result.status !== "ok") {
            const msg = (result && result.message) || "unknown error";
            rollbackPreview();
    ...
    function stopNativePointer(ev) {
      ...
      else if (ev && typeof ev.stopPropagation === "function")
        ev.stopPropagation();
    }

    function capturePointerDrag(ev) {
      const rect =
        typeof slider.getBoundingClientRect === "function"
          ? slider.getBoundingClientRect()
          : null;
      const width =
        rect && Number.isFinite(Number(rect.width)) && Number(rect.width) > 0
          ? Number(rect.width)
          : 240;
      const rectLeft =
        rect && Number.isFinite(Number(rect.left)) ? Number(rect.left) : 0;
      const screenX = Number(ev && ev.screenX);
      const clientX = Number(ev && ev.clientX);
      const left =
        Number.isFinite(screenX) && Number.isFinite(clientX)
    ...
    function pointerMatchesDrag(ev) {
      ...
      if (pointerDrag.pointerId === undefined || pointerDrag.pointerId === null)
        return true;
    ...
    function textScalePctFromPointer(ev) {
      ...
      const normalized = Math.max(
        0,
        Math.min(1, (x - pointerDrag.left) / pointerDrag.width),
      );
      return snapTextScalePct(
        TEXT_SCALE_UI_MIN +
          normalized * (TEXT_SCALE_UI_MAX - TEXT_SCALE_UI_MIN),
      );
    }
    ...
    function markNativeChangeSuppressed(pct) {
      suppressNativeChange = {
        pct: snapTextScalePct(pct),
        until: Date.now() + 500,
      };
    }
    ...
    const unsubscribeContextChanged =
      window.settingsAPI &&
      typeof window.settingsAPI.onTextScaleContextChanged === "function"
  ...
  function buildSizeSliderRow() {
    ...
    function setDragging(
      nextDragging,
      pending = state.transientUiState.size.pending,
    ) {
    ...
    const initial =
      state.transientUiState.size.draftUi === null
        ? readers.readSizeUiFromSnapshot()
        : state.transientUiState.size.draftUi;
    applyLocalValue(initial);
    setDragging(
      state.transientUiState.size.dragging,
      state.transientUiState.size.pending,
    );
  ...
  function hasMountedBubblePolicyControls() {
    ...
    if (!summaryControl || !document.body.contains(summaryControl.element))
      return false;
  ...
  function patchInPlace(changes) {
    ...
    if (
      keys.includes("size") &&
      !ops.syncMountedSizeControl({ fromBroadcast: true })
    )
      return false;
    ...
    if (
      keys.includes("sessionHudEnabled") &&
      !SESSION_HUD_CHILD_SWITCH_KEYS.every((key) =>
        getMountedGeneralSwitch(key),
      )
    ) {
      return false;
    }
    if (
      (keys.includes("hideBubbles") ||
        keys.some((key) => BUBBLE_POLICY_KEYS.has(key))) &&
    ...
    for (const key of keys) {
      ...
      if (SESSION_CLEANUP_NUMBER_KEYS.has(key)) {
        state.mountedControls.sessionCleanupControls
          .get(key)
          .syncFromSnapshot();
        continue;
      }
      ...
      helpers.setSwitchVisual(
        meta.element,
        readers.readGeneralSwitchVisual(key, meta.invert),
        { pending: false },
      );
      if (key === "soundMuted") {
    ...
    if (
      keys.includes("sessionHudEnabled") &&
      !syncSessionHudChildSwitchesDisabled()
    )
      return false;
    if (keys.some((key) => SESSION_HUD_SUMMARY_KEYS.has(key))) {
      const summary = state.mountedControls.sessionHudSummary;
      if (summary && document.body.contains(summary.element))
        summary.syncFromSnapshot();
    }
    if (
      (keys.includes("hideBubbles") ||
        keys.some((key) => BUBBLE_POLICY_KEYS.has(key))) &&
      !syncBubblePolicyControlsFromSnapshot()
    )
      return false;
    if (
      (keys.includes("soundVolume") || keys.includes("soundMuted")) &&
