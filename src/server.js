...
module.exports = function initServer(ctx) {
  ...
  const CORS_ORIGINS =
    process.env.CLAWD_CORS_ORIGINS === "*"
      ? "*"
      : (
          process.env.CLAWD_CORS_ORIGINS ||
  ...
  const mobileIntegration =
    ctx.mobileCompanionEnabled !== false
  ...
  const getMobileWS = mobileIntegration
    ? mobileIntegration.getMobileWS
    : () => null;
  const getMobileToken = mobileIntegration
    ? mobileIntegration.getMobileToken
    : () => null;
  const getPendingMobileApprovals = mobileIntegration
    ? mobileIntegration.getPendingMobileApprovals
    : () => [];
  const saveMobileState = mobileIntegration
    ? mobileIntegration.saveMobileState
    : () => {};
  const broadcastHookEvent = mobileIntegration
    ? mobileIntegration.broadcastHookEvent
    : () => {};
  const startMobileServerBase = mobileIntegration
    ? mobileIntegration.startMobileServer
    : () => {};
  ...
  function recordHookEvent(data, route, outcome) {
    return recordHookEventInBuffer(recentHookEvents, data, route, outcome, {
      now: nowFn,
    });
  }

  function createRequestHookRecorder(data, defaultRoute) {
    return createSingleRequestHookEventRecorder(
      recordHookEvent,
      data,
      defaultRoute,
    );
  ...
  function clearRecentHookEvents(agentId) {
    if (typeof agentId === "string" && agentId)
      recentHookEvents.delete(agentId);
  ...
  function getRuntimeStatus() {
    ...
    try {
      address =
        httpServer && typeof httpServer.address === "function"
          ? httpServer.address()
          : null;
    } catch {
      address = null;
    ...
    return {
      ...
      port,
      runtimePath:
        typeof ctx.runtimeConfigPath === "string"
  ...
  function getClaudeHookGuardStatus() {
    ...
    if (
      nowFn() - lastClaudeHookGuardNotice.at >
      CLAUDE_HOOK_GUARD_NOTICE_TTL_MS
    )
      return null;
  ...
  function clearClaudeHookGuardAfterClaudeSync(agentId, result) {
    if (shouldClearClaudeHookGuardAfterSync(agentId, result))
      clearClaudeHookGuardStatus();
    return result;
  }

  function syncIntegrationForAgent(agentId) {
    return clearClaudeHookGuardAfterClaudeSync(
      agentId,
      syncIntegrationForAgentBase(agentId),
    );
  }
  ...
  function repairIntegrationForAgent(agentId, options = {}) {
    return clearClaudeHookGuardAfterClaudeSync(
      agentId,
      repairIntegrationForAgentBase(agentId, options),
    );
  }
    ...
    return new Promise((resolve) => {
      ...
      httpServer.on("error", (err) => {
        ...
        if (
          !activeServerPort &&
          err.code === "EADDRINUSE" &&
          listenIndex < listenPorts.length - 1
        ) {
          listenIndex++;
        ...
        if (!activeServerPort && err.code === "EADDRINUSE") {
          ...
          console.warn(
            `Ports ${firstPort}-${lastPort} are occupied — state sync and permission bubbles are disabled`,
          );
        } else {
      ...
      httpServer.on("listening", () => {
        ...
        console.log(
          `Clawd state server listening on ${logHost}:${activeServerPort}`,
        );
        console.log(
          `  Mobile companion token: ${getMobileToken().slice(0, 4)}… (set via QR scan or settings page)`,
        );
